...
- SQL 쿼리문은 SELECT, FROM, WHERE 문법으로 작성
...
- 데이터가 여러 장소에 저장되어 있다면 -> 특정 Table에 있는 데이터를 각각 추출 후 연결

- AS는 별칭을 지어줄 때
- 컬럼 이름에 따옴표를 넣는 경우: 자주하는 실수
...
- SELECT 집계할 컬럼1, 집계함수 (COUNT, MAX, MIN)
- FROM Table
- GROUP BY 집계할_컬럼1

- **DISTINCT**: 고유값을 알고 싶은 경우 -> DISTINCT는 중복 값을 제거하는 것

<그룹화 활용 포인트>
- 일자별 집계
- 연령대별 집계 (특정 연력대에서 더 많이 구매했는가?)
- 특정 타입별 집계
- 앱 화면별 집계

- **WHERE**: Table에 바로 조건을 사용하고 싶은 경우
- **HAVING**: GROUP BY 한 후 설정하고 싶은 경우 사용

- 정렬하기: ORTHER BY
SELECT col FROM ORDER BY <컬럼> <순서>
